...
class Marketplace(sp.Contract):
    ...
    def collect(self, params):
        ...
        current_holder = self.data.data[params.token_id].holder
        

        #sending rewards
        sp.send(self.data.data[params.token_id].author, sp.split_tokens(sp.amount, 97, 100))
        
        self.fa2_transfer(self.data.token, current_holder, sp.sender, params.token_id, 1)
        self.data.data[params.token_id].holder = sp.sender
    @sp.entry_point
    def update_token(self, params):
        sp.verify(self.data.data[params.token_id].holder == sp.sender)
...
def test():
    ...
    scenario.h1("update")
    scenario += marketplace.update_token(sp.record(token_id = 1)).run(sender = elon)
    scenario += marketplace.collect(sp.record(token_id = 1)).run(sender = elon2, amount = sp.mutez(5600000))
